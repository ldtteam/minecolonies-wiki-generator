...
public class CrafterRecipeDataGenerator extends DataGenerator<ClientLevel>
{
    ...
    public CompletableFuture<Void> generate(final GeneratorTarget target, final DataGeneratorOptions<ClientLevel> options)
    {
        return CompletableFuture.runAsync(() -> {
            final ResourceLocation fileKey = CRAFTER_RECIPES.idToFile(ResourceLocation.fromNamespaceAndPath(target.namespace(), target.path()));
            ...
            try
            {
                generateCrafterRecipeData(options, target, resource);
            }
            catch (Exception e)
            {
    ...
    private void generateCrafterRecipeData(final DataGeneratorOptions<ClientLevel> options, final GeneratorTarget target, final Resource resource) throws IOException
    {
