...
public class ConfigurationDataGenerator extends DataGenerator<ClientLevel>
{
    ...
    public Set<GeneratorTarget> listTargets(final ClientLevel level)
    {
        return ModList.get().getMods().stream()
            ...
            .filter(modId -> !ModConfigs.getModConfigs(modId).isEmpty())
    ...
    public CompletableFuture<Void> generate(final GeneratorTarget target, final DataGeneratorOptions<ClientLevel> options)
    {
        return CompletableFuture.runAsync(() -> {
            ...
            final Map<ModConfig.Type, Map<LinkedList<String>, ModConfigSpec.ValueSpec>> configurationTypes = new LinkedHashMap<>();

            ModConfigs.getModConfigs(modId).forEach(config -> {
            ...
            try
            {
                ...
                for (final Map.Entry<ModConfig.Type, Map<LinkedList<String>, ModConfigSpec.ValueSpec>> entry : configurationTypes.entrySet())
                {
                    ...
                    for (final Map.Entry<LinkedList<String>, ModConfigSpec.ValueSpec> valueEntry : entry.getValue().entrySet())
                    {
